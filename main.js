...
function getPlayableBounds(){
  const rect = stage.getBoundingClientRect();
  const left = rect.width * SAFE_BOUNDS.left;
  const right = rect.width * (1 - SAFE_BOUNDS.right);
  const top = rect.height * SAFE_BOUNDS.top;
  const bottom = rect.height * (1 - SAFE_BOUNDS.bottom);
  return { rect, left, right, top, bottom };
}

function clampPointToPlayableArea(x, y){
...
function clampToStage(){
  const p = clampPointToPlayableArea(state.player.x, state.player.y);
  state.player.x = p.x;
  state.player.y = p.y;
}
...
function setTargetFromEvent(e){
  ...
  const p = clampPointToPlayableArea(x, y);
...
function respawnPlayer(){
  ...
  state.player.x = state.player.spawnX;
  state.player.y = state.player.spawnY || (b.bottom - 10);

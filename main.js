const VER = "na-11"; // поменяй строку если GitHub Pages кэшит
...
const DEVICE_PROFILE = (() => {
  ...
  return mobile
    ? {
        ...
        playerScaleMul: 0.52,
        dragonScaleMul: 2.0,
        depth: { topScale: 0.12, midScale: 0.20, bottomScale: 0.36 },
      }
    : {
        mobile: false,
        playerScaleMul: 0.62,
        dragonScaleMul: 1.5,
        depth: { topScale: 0.18, midScale: 0.32, bottomScale: 0.58 },
      };
})();
...
// ====== DEPTH + HITBOX ======
const DEPTH = DEVICE_PROFILE.depth;
...
function clampToStage(){
  ...
  state.player.x = Math.max(marginSide, Math.min(rect.width - marginSide, state.player.x));
  state.player.y = Math.max(marginTop, Math.min(rect.height - marginBottom, state.player.y));

  if (state.player.targetX != null){
    state.player.targetX = Math.max(marginSide, Math.min(rect.width - marginSide, state.player.targetX));
  }
  if (state.player.targetY != null){
    state.player.targetY = Math.max(marginTop, Math.min(rect.height - marginBottom, state.player.targetY));
  }
}
...
function resize(){
  ...
  state.player.spawnX = Math.round(w * 0.16);
  state.player.spawnY = Math.round(h * 0.965);
...
function placeEntities(){
  ...
  const ps = scaleForY(state.player.y) * DEVICE_PROFILE.playerScaleMul;
  const ds = scaleForY(state.dragon.y) * DEVICE_PROFILE.dragonScaleMul;
